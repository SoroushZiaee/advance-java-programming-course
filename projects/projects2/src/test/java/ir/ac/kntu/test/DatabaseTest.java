...
public class DatabaseTest {
    ...
    public void getStudentsTest(){
        assertEquals(false, db.getStudents().isEmpty());
    }

    @Test
    public void getProfsTest(){
        System.out.println(db.getProfs());
        assertEquals(false, db.getProfs().isEmpty());
    }

    @Test
    public void getLessonGroups(){
        assertEquals(false, db.getLessonGroups().isEmpty());
    }

    @Test
    public void removeStudentTest(){
        int sizeBefore = db.getStudents().size();
        db.removeStudent(9728143);
        assertEquals(sizeBefore - 1, db.getStudents().size());
    }

    @Test
    public void removeProfTest(){
        int sizeBefore = db.getProfs().size();
        db.removeProf(1);
        assertEquals(sizeBefore - 1, db.getProfs().size());
    }

    @Test
    public void removeGroupLessonTest(){
        int sizeBefore = db.getLessonGroups().size();
        db.removeGroupLesson(1);
        assertEquals(sizeBefore - 1, db.getLessonGroups().size());
    }

    @Test
    public void getStudentByIdTest(){
        Student studentExpected = db.getStudents().get(0);
        Student studentTest = db.getStudentById(studentExpected.getId());

        assertEquals(studentExpected.getId(), studentTest.getId());
    }

    @Test
    public void getProfessorByIdTest(){
        Prof professorExpected = db.getProfs().get(0);
        Prof professorTest = db.getProfessorById(professorExpected.getId());

        assertEquals(professorExpected.getId(), professorTest.getId());
    }

    @Test
    public void getLessonGroupByIdTest(){
        LessonGroup lessonGroupExpected = db.getLessonGroups().get(0);
        LessonGroup lessonGroupTest = db.getLessonGroupById(lessonGroupExpected.getLessonGroupId());

        assertEquals(lessonGroupExpected.getLessonGroupId(), lessonGroupTest.getLessonGroupId());
    }

    @Test
    public void updateStudentTest(){
        Student student = db.getStudents().get(0);
        student.setName("new Name");
        db.removeStudent(student.getId());
        db.updateStudent(student);
        assertEquals(true, student.getName().equals(
                db.getStudents().get(db.getStudents().size() - 1).getName()
        ));
    }
}
